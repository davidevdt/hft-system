...
#pragma pack(push,1)
    ...
    // Provides market data updates to the data publishing component 
    struct MEMarketUpdate {
        MarketUpdateType type_ = MarketUpdateType::INVALID; 
        OrderId order_id_ = OrderId_INVALID; // orderd id in the limit order book for which the order update is applicable 
        TickerId ticker_id_ = TickerId_INVALID; 
        Side side_ = Side::INVALID; 
        Price price_ = Price_INVALID; 
        Qty qty_ = Qty_INVALID; 
        Priority priority_ = Priority_INVALID; // specifies the position of the order in the FIFO queue 
        auto toString() const {
            std::stringstream ss; 
            ss << "MEMarketUpdate"
               << " [" 
